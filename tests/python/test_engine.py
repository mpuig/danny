...
class EngineContractTests(unittest.TestCase):
    ...
    def test_question_ids_do_not_enter_prompts_or_model_identity(self):
        ...
        response = engine.respond({"model": "jev-latest", "state": {"a": ["b"]}, "questions": {"first": q}})
        ...
        for request in [[], {}, {"state": "s", "questions": []},
                        {"state": "s", "questions": {}}, {"state": "s", "questions": {"a": None}}]:
    ...
    def test_mutated_cache_is_not_retried(self):
        ...
        engine.model = Model()
        cache = [SimpleNamespace(keys=mx.zeros((1, 1, 12, 1)), values=mx.zeros((1, 1, 12, 1)))]
        ...
        self.assertEqual(engine.model.calls, 1, "only prefix call; must not retry the suffix")


@unittest.skipUnless(HAS_MLX and os.environ.get("JEV_TEST_MODEL"), "set JEV_TEST_MODEL for local model tests")
class ModelParityTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.engine = SystemOneEngine(os.environ["JEV_TEST_MODEL"], renderer_version=STRUCTURED_V1)

    def test_mixed_batch_padding_and_question_invariance(self):
        state = {"ticket": {"messages": ["The package arrived damaged. Please refund it."]}}
        questions = {
            "refund": Question("noul", "Does `ticket.messages[0]` request a refund?"),
            "team": Question("choice", "Which department fits?", {
                "returns": {"covers": ["refunds", "damaged products"]},
                "sales": "New purchases", "account": None,
            }),
            "tone": Question("score", "How frustrated is the customer?", ["Calm", "Frustrated", "Very angry"]),
        }
        engine = self.engine
        ...
        for actual, expected, reversed_actual in zip(batched, sequential, reversed_batch):
        ...
        renamed = engine.ask(state, {"unrelated_id": questions["refund"]})["unrelated_id"].noul
